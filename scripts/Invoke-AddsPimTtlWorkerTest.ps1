...
param(
    ...
    [ValidateSet('PowerShell', 'Ldap')] [string] $Adapter = 'PowerShell',
    [ValidateRange(60, 3600)] [long] $RequestedTtlSeconds = 900
)

$ErrorActionPreference = 'Stop'
$taskName = 'ADDS-PIM-OneTime-TtlTest'
$workerScript = Join-Path $PSScriptRoot '..\src\ADDS.PIM.AdWorker\Scripts\Invoke-TtlMembership.ps1'
$workerScript = (Resolve-Path $workerScript).Path
...
try {
    ...
    if ($Adapter -eq 'PowerShell') {
        $arguments = "-NoProfile -NonInteractive -ExecutionPolicy AllSigned -File `"$workerScript`" -DomainController $DomainController -TargetAccountObjectGuid $TargetAccountObjectGuid -TargetGroupObjectGuid $TargetGroupObjectGuid -RequestedTtlSeconds $RequestedTtlSeconds"
